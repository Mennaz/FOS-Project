...
{
uint32*Fisrt_Va_Heap=kmalloc(PAGE_SIZE);
uint32 Phy_Add =kheap_physical_address((int)Fisrt_Va_Heap);
Phy_Add=Phy_Add| PERM_PRESENT | PERM_USER| PERM_WRITEABLE;
ptr_page_directory[PDX(virtual_address)]=Phy_Add;
for(int i=0; i<(PAGE_SIZE/4); i++)
{
Fisrt_Va_Heap[i]=0;
}
tlbflush();
return Fisrt_Va_Heap;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	size = ROUNDUP(size,PAGE_SIZE);
	Pages=(size/PAGE_SIZE);
			uint32 First_Va=virtual_address;
        int i = 0;
			while(i<Pages){
				pf_remove_env_page(e,First_Va);
				First_Va +=PAGE_SIZE;
				i++;
			}
			...
			uint32 *Ptr_Page_Table;
			First_Va=virtual_address;
			uint32 Last_Va =(virtual_address+ROUNDUP(size,PAGE_SIZE));
			...
			{
				Ptr_Page_Table=NULL;
				get_page_table(e->env_page_directory,(void*)First_Va,&Ptr_Page_Table);
				if(Ptr_Page_Table !=NULL)
				{
					...
					if(count == 1024)
					{
						...
						pd_clear_page_dir_entry(e,(uint32)First_Va);
					}
				}
				First_Va+=PAGE_SIZE;
			}
