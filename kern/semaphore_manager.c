...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	for(int i=0; i < MAX_SEMAPHORES; i++)
		...
		struct  Semaphore *s;
		int x =allocate_semaphore_object(&s);
		if(x!=E_NO_SEMAPHORE)
		{
			semaphores[x].ownerID=ownerEnvID;
			strcpy(semaphores[x].name,semaphoreName);
			semaphores[x].value=initialValue;
		}
		return x;
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int ID;
	ID = get_semaphore_object_ID(ownerEnvID ,semaphoreName );
		semaphores[ID].value = semaphores[ID].value - 1 ;
		if(semaphores[ID].value <0 )
		{
			sched_remove_ready(curenv);
			enqueue(&semaphores[ID].env_queue,curenv);
