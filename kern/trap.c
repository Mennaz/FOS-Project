...
struct Least_Recently_Used arr[100];
int count=0;
void  Placement(struct Env * curenv, uint32 fault_va,int index)
{
   uint32 Entry;
   uint32 *ptr_page_table1;
   uint32 *ptr_page_table2;
   uint32 *ptr_page_table3;
   ...
   get_page_table(curenv->env_page_directory,(void*)fault_va,&ptr_page_table1);
   ...
   if(ret==E_PAGE_NOT_EXIST_IN_PF)
   {
    ...
    if(curenv->page_WS_max_size>curenv->page_last_WS_index)
    {
     ...
     env_page_ws_set_entry(curenv,curenv->page_last_WS_index,Rounded_VA);
...
int i=0;
int Replacement(struct Env * curenv, uint32 fault_va)
{
	 int index;
     ...
     while(1)
	 {
	    ...
	    va=curenv->ptr_pageWorkingSet[index].virtual_address;
		get_page_table(curenv->env_page_directory,(void*)va,&ptr_page_table1);
		entry=ptr_page_table1[PTX(va)];
		if((entry&PERM_USED)!=PERM_USED)
		{
			if((entry&PERM_MODIFIED)==PERM_MODIFIED)
			{
				 struct Frame_Info* ptr_frame_info = get_frame_info(curenv->env_page_directory,(void*)va, &ptr_page_table2);
		    ...
		    Placement(curenv,fault_va,curenv->page_last_WS_index);
			return 1;
	    }
		else
		{
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	else
	{
	  ...
	  {
		Replacement(curenv,fault_va);
		return;
	  }
	  if(isPageReplacmentAlgorithmFIFO())
	 	{
	 	    get_page_table(curenv->env_page_directory,(void*)curenv->ptr_pageWorkingSet[index].virtual_address,&ptr_page_table1);
	 	    uint32 entry=ptr_page_table1[PTX(curenv->ptr_pageWorkingSet[index].virtual_address)];
	 	    if((entry&PERM_MODIFIED)==PERM_MODIFIED)
	 	     {
	 	       struct Frame_Info* ptr_frame_info = get_frame_info(curenv->env_page_directory,(void*)curenv->ptr_pageWorkingSet[index].virtual_address, &ptr_page_table1);
	 	       pf_update_env_page(curenv,(void*)curenv->ptr_pageWorkingSet[index].virtual_address,ptr_frame_info);
	 	     }
	 	     unmap_frame(curenv->env_page_directory,(void*)curenv->ptr_pageWorkingSet[index].virtual_address);
	 	     Placement(curenv,fault_va,index);
	          index++;
	         if(index==env_page_ws_get_size(curenv))
	             index=0;
	 	 }
	  if(isPageReplacmentAlgorithmModifiedCLOCK())
	  {
		 ...
		 while (True)
	           ...
	           for(int j=0;j<curenv->page_WS_max_size;j++)
	          {
		        ...
		        va=curenv->ptr_pageWorkingSet[index].virtual_address;
		        get_page_table(curenv->env_page_directory,(void*)va,&ptr_page_table1);
		        uint32 entry=ptr_page_table1[PTX(va)];
		       if((entry&PERM_USED)!=PERM_USED && (entry&PERM_MODIFIED)!=PERM_MODIFIED)
		      	{
		        	unmap_frame(curenv->env_page_directory,(void*)va);
		         	pt_set_page_permissions(curenv, fault_va,PERM_USED,0);
		            Placement(curenv,fault_va,curenv->page_last_WS_index);
		...
		if(isPageReplacmentAlgorithmLRU())
		{
		   for(int j=0;j<curenv->page_WS_max_size;j++)
		   {
		     arr[j].time_stamp= env_page_ws_get_time_stamp(curenv,j);
		   }
		   uint32 min=arr[0].time_stamp;
		   for(int k=0;k<env_page_ws_get_size(curenv);k++)
		   {
		      if(arr[k].time_stamp<min)
		      {
		        min =arr[k].time_stamp;
		        save_time=k;
		      }
		   }
	       get_page_table(curenv->env_page_directory,(void*)curenv->ptr_pageWorkingSet[save_time].virtual_address,&ptr_page_table2);
	       uint32 entry=ptr_page_table2[PTX(curenv->ptr_pageWorkingSet[save_time].virtual_address)];
	       if((entry&PERM_MODIFIED)==PERM_MODIFIED)
	        {
	         struct Frame_Info* ptr_frame_info = get_frame_info(curenv->env_page_directory,(void*)curenv->ptr_pageWorkingSet[save_time].virtual_address, &ptr_page_table2);
	         pf_update_env_page(curenv,(void*)curenv->ptr_pageWorkingSet[save_time].virtual_address,ptr_frame_info);
		     }
	       unmap_frame(curenv->env_page_directory,(void*)curenv->ptr_pageWorkingSet[save_time].virtual_address);
	       Placement(curenv,fault_va,save_time);
	      }
	    }
}
